...
async def verify_new_round_events(event_system, expected_new_data_num, expected_new_prev_id, consensus_data):
    event = await get_event(event_system)
    verify_new_data_event(BroadcastConsensusDataEvent, event, expected_new_data_num,
                          expected_new_prev_id, consensus_data)
    event = await get_event(event_system)
    verify_new_data_event(ReceivedConsensusDataEvent, event, expected_new_data_num,
                          expected_new_prev_id, consensus_data)
    event = await get_event(event_system)

    # await verify_new_vote_event(event_type=BroadcastConsensusVoteEvent,
    #                             event=event,
    #                             expected_round_num=,
    #                             vote_term_num=,
    #                             vote_commit_id=)
    # await verify_new_vote_event(event_type=ReceivedConsensusVoteEvent,
    #                             event=event,
    #                             expected_round_num=,
    #                             vote_term_num=,
    #                             vote_commit_id=)
...
def verify_new_data_event(event_type, event, expected_new_data_num, expected_new_prev_id, consensus_data):
    assert isinstance(event, event_type)
    verify_new_consensus_data(event.data, expected_new_prev_id, expected_new_data_num)
    if expected_new_data_num == 2:
        verify_prev_votes(event.data.prev_votes, consensus_data)
...
def verify_prev_votes(prev_votes, consensus_data):
    prev_voters = []
    assert len(prev_votes) >= QUORUM
    for vote in prev_votes:
        assert vote.data_id == consensus_data.id
        assert vote.commit_id == consensus_data.prev_id
        assert vote.voter_id not in prev_voters
        prev_voters.append(vote.voter_id)


async def verify_new_vote_event(event_type, event, expected_round_num, vote_term_num, vote_commit_id):
    assert isinstance(event, event_type)
